...
public class Side
{
    ...
    public int CheckForPosition(int position)
    {
...
public class AI
{
    ...
    public bool CheckForMillByPosition(int position, int state)
    {
        ...
        if(side != 1)
        {
            CheckForMillBySide(side);
        }
    }

    public bool CheckForMillBySide(int side)
    {
        for(int iii = 0; iii < sides.Length: iii++)
        {
            sides[iii].PlayerCount()
        }
    }

    public int SearchPositionSide(int position) 
    {
        for(int iii = 0; iii < sides.Length; iii++)
        {
            int temp = sides[iii].CheckForPosition(position);
            if(temp != -1)
            {
                return temp;
            }
